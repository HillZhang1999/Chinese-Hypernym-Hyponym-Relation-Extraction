...
class SyntacticExtraction_contain(father.SyntacticExtraction_isA):
    ...
    def compose(self,subjs, objs):
        result = []
        if (subjs == -1) or (objs == -1):  # 上下位中有一个未抽取到词
            return result
        for subj in subjs:
            if (subj[1] == 'NN' or subj[1] == 'NR'):  # 是否名词（时间名词除外）
                for obj in objs:
                    if (obj[1] == 'NN' or obj[1] == 'NR'):  # 是否名词（时间名词除外）
                        result.append(obj[0] + ' ' + subj[0])
        return result
            ...
            for i in range(len(ycjf)):
                ...
                if (tmp[2] == 'root') and (eval(tmp[3]) > limit):  # 谓语做root的情况（需要先找主语）
                    ...
                    if (st != words[int(tmp[0]) - 1]) and ('*' not in st) and ('《' not in st):
                        ...
                        subjs.append([words[int(tmp[0]) - 1], cx[int(tmp[0]) - 1]])
                    else:
                        subjs.append([st, cx[int(tmp[0]) - 1]])
                    if (self.isPattern(words, tmp[1], ycjf, 0)):
                        ycjf[i] = 'visited'  # 防止重复访问结点
...
def main():
    extraction = SyntacticExtraction_contain()
    #extraction.saveWordPairs('wiki_test_data', 'WordPairs_ycjf_contain_wiki_profix_3.txt', 0)
    extraction.saveWordPairs('/data/zhangyue19/tolukaihua/baike_data0.out','WordPairs_ycjf_contain_baike0_profix_inDic.txt',0)
    extraction.saveWordPairs('/data/zhangyue19/tolukaihua/baike_data1.out', 'WordPairs_ycjf_contain_baike1_profix_inDic.txt', 0)
    extraction.saveWordPairs('/data/zhangyue19/tolukaihua/baike_data2.out', 'WordPairs_ycjf_contain_baike2_profix_inDic.txt', 0)
    extraction.saveWordPairs('/data/zhangyue19/tolukaihua/lter_wiki_data_ws_pos_standrad.out', 'WordPairs_ycjf_contain_wiki_profix_inDic.txt', 0)
    #chooseWordPairs('WordPairs-ycjf.txt', 'ycjf-result.txt', 'words.conll')
    #print(len(readFile('WordPairs-isA-ycjf.txt')))
#主函数
